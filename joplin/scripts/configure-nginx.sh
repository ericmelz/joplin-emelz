...
server {
    ...
    # Proxy configuration
    location / {
        proxy_pass http://traefik.$DOMAIN;
...
EOF
    ...
    if certbot certonly --webroot -w /var/www/html -d "$DOMAIN" --non-interactive --agree-tos --email "eric@emelz.org"; then
...
# Main execution
main() {
    local saved_enable_ssl="$ENABLE_SSL"
    ...
    if ! $DRY_RUN; then
	# Don't enable SSL the first time around since that config generates references to certs
	info "Generating config without SSL..."
	ENABLE_SSL=false
        create_nginx_config
	ENABLE_SSL=$saved_enable_ssl
        enable_site
        test_nginx_config
        ...
        if $ENABLE_SSL; then
            ...
            info "Certificate obtained.  Generating config with SSL.."
            create_nginx_config
            test_nginx_config
        fi
